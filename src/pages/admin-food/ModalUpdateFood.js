...
const ModalUpdateFood = ({ open, setOpen, setFoods }) => {
  ...
  const onSubmit = data => {
    if (!file && !open?.picture) return toast.error('Hãy thêm ảnh cho món của bạn')

    setLoading(true)
    let imageRef = null
    if (file) imageRef = ref(storage, `images/${user._id}-${new Date().valueOf()}`)
    ...
    if (file)
      uploadBytes(imageRef, file)
        ...
        .then(({ data }) => {
          if (data.isSuccess) {
            setFoods(prev => prev.map(food => (food._id === data.data.food?._id ? data.data.food : food)))
  ...
  return (
    <Dialog
      ...
      fullWidth
    >
      <DialogTitle id='add-food'>Cập nhật món</DialogTitle>
          ...
          <Grid container spacing={5}>
            ...
            <Grid item xs={12} style={{ position: 'relative', cursor: 'pointer' }}>
              <Box {...getRootProps({ className: 'dropzone' })} sx={file ? { maxHeight: 200 } : {}}>
                ...
                {file ? (
                  <img
                    key={file?.name || open?.name}
                    alt={file?.name || open?.name}
